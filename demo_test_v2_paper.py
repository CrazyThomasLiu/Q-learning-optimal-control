...
for time in range(batch_length):
    ...
    #pdb.set_trace()
#"load the control data from the PI-indirect ILC"
#df_y_ilc = pd.read_csv('./comparison_algorithm/PI_Robust_output.csv', index_col=0)
#y_ilc  = df_y_ilc.to_numpy()
#df_u_ilc = pd.read_csv('./comparison_algorithm/PI_Robust_control_signal.csv', index_col=0)
#u_ilc  = df_u_ilc.to_numpy()
"7. plot the figures"
...
#ax1.plot(time, u_ilc, label='smoothed')
xlable = 'Time:$t$'
...
if save_figure==True:
    ...
    #plt.savefig('Q_learing.jpg',dpi=700)
    plt.savefig('Q_learing.pdf')
plt.show()
